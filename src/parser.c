...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[sampling]")==0) return SAMPLING;
    return BLANK;
}
...
layer parse_sampling(list *options, size_params params)
{
    float beta = option_find_float_quiet(options, "beta", 0.5);
    layer l = make_sampling_layer(params.batch, params.inputs, params.inputs / 2, beta);
    l.h = l.out_h = params.h;
    l.w = l.out_w = params.w;
    l.c = l.out_c = params.c;
    return l;
...
network *parse_network_cfg(char *filename)
{
    ...
    while(n){
        ...
        if(lt == CONVOLUTIONAL){
            ...
            l = parse_sampling(options, params);
        }
